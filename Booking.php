...
// Assuming this page corresponds to a specific tour, you would already know the tour_id.
$tour_id = 4;  // For example, tour_id for this package
$tour_name = "Spend your Holiday in Akagera";  // Tour name
$tour_image = "assets/images/lodge.jpg";    // Tour image
...
// Clone the date and add intervals for the tours
$tourStart1 = clone $currentDate;
$tourStart1->modify('+10 days'); // Tour 1 starts 10 days from today
$tourEnd1 = clone $tourStart1;
$tourEnd1->modify('+5 days');    // Tour 1 ends 5 days after it starts

$tourStart2 = clone $currentDate;
$tourStart2->modify('+20 days'); // Tour 2 starts 20 days from today
$tourEnd2 = clone $tourStart2;
$tourEnd2->modify('+5 days');    // Tour 2 ends 5 days after it starts

// Format the dates to be displayed
$formattedStart1 = $tourStart1->format('d F, Y');
$formattedEnd1 = $tourEnd1->format('d F, Y');
$formattedStart2 = $tourStart2->format('d F, Y');
$formattedEnd2 = $tourEnd2->format('d F, Y');
...
<title>Akagera Travel. – AcrossAfricaJourneys</title>
...
<h1>
	Spend Your Holidays In Akagera. Embracing Wildlife Animals .
</h1>
<ul id="breadcrumb-list" class="breadcrumb-list"><li class="breadcrumb-item"><a href="https://triprex.egenslab.com"> Home </a></li><li class="active">Discover Serenity, Exploration, and Enlightenment.</li></ul> </div>
</div>
</div>
</div>
</div>
...
<p>As the only savannah park, Akagera National Park is located in Rwanda. Game drives provide a chance to view a variety of animals, birds, and stunning scenery. The Akagera National Park in eastern Rwanda is home to over 500 bird species as well as a wide range of mammals, including the Big 5 of Africa (lions, leopards, rhinoceroses, elephants, and Cape buffalo). Other animals to keep an eye out for include giraffes, bushbucks, hippos, impalas, topis, zebras, and many more.    </p>
...
<span>Day 01 :</span> DRIVE FROM KIGALI TO AKAGERA NATIONAL PARK </button>
</h2>
<div id="collapse0" class="accordion-collapse collapse show" aria-labelledby="heading0" data-bs-parent="#tourPlan">
<div class="accordion-body">
<p>Pick-up and transfer to Akagera national park, it is three hours driving from Kigali including stopover, on the way we will enjoy the scenic and windy road surrounded by the green endless rolling hills and we will have some short stopover for the cup of coffee or the restroom use.
	Arriving at the park’s reception to proceed the payments, later we start a short game drive within the park as we are heading to the hotel for check in. over night Ruzizi Tented Lodge 
	</p>
<ul class="exclud">
...
<span>Day 02 :</span> AKAGERA FULL GAME DRIVE TRANSFER BACK TO KIGALI.    </button>
</h2>
<div id="collapse1" class="accordion-collapse collapse " aria-labelledby="heading1" data-bs-parent="#tourPlan">
<div class="accordion-body">
<p>After breakfast, we will go on a game drive from the north to the south of the park, enjoying the amazing views and many exotic animals that the area has to offer. We will also make some stops along the way for lunch and restroom breaks.  Later, we will leave the park via the southern entrance and return to Kigali.</p>
<ul class="exclud">
<li><i class="bi bi-check-lg"></i> Admire Big Ben, Buckingham Palace And St Paul’s Cathedral</li>
<li><i class="bi bi-check-lg"></i> Chance To Spot Prominent Landmarks Of The City</li>
</ul>
...
<span>Day 03 :</span> BOAT CRUISE IN LAKE IHEMA AND  </button>
</h2>
<div id="collapse2" class="accordion-collapse collapse " aria-labelledby="heading2" data-bs-parent="#tourPlan">
<div class="accordion-body">
<p>Wrap-up and drive to Lake Ihema for a boat tour in the morning after breakfast. This boat ride provides you the opportunity to spot some Hippos and Crocodiles from a nice angle, as well as other creatures such as elephants and others. Later driver to the north of  the park over night at Karenge bush camp </p>
<ul class="exclud">
</ul>
</div>
</div>
...
<span>Day 05:</span> Return </button>
</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const customDateGroup = document.getElementById('custom_date');
...
// Disable custom date input unless custom radio button is selected
$('#custom_date_input').prop('disabled', true);
    
$('input[name="Booking_date"]').on('change', function() {
	if ($('#customDateRadio').is(':checked')) {
		$('#custom_date_input').prop('disabled', false);  // Enable custom date input
	} else {
		$('#custom_date_input').prop('disabled', true);   // Disable custom date input
	}
});
...
document.querySelectorAll('input[name="Booking_date"]').forEach(function (radio) {
...
<script>
  // Base prices for adults and children
  const adultPrice = 340;
  const childPrice = 20;
  const servicePrices = {
		home_pickup: 10,
		night_food: 15,
	...
	// Elements for number of adults, children, and total price
	const adultInput = document.querySelector('input[name="adults"]');
	const childInput = document.querySelector('input[name="children"]');
	const serviceCheckboxes = document.querySelectorAll('.services_check');
	const totalPriceElement = document.querySelector('.cart_total_price');

  // Function to calculate the total price
  function updateTotalPrice() {
	// Get the current number of adults and children
	const adultCount = parseInt(document.querySelector('input[name="adults"]').value) || 1;
	const childCount = parseInt(document.querySelector('input[name="children"]').value) || 0;
	...
	serviceCheckboxes.forEach((checkbox) => {
			if (checkbox.checked) {
				const serviceSlug = checkbox.getAttribute('service-slug');
				totalPrice += servicePrices[serviceSlug];
			}
		});
	...
	// Event listeners to trigger the price calculation
  ...
  // Function to handle the increment and decrement of quantity
  function handleQuantityChange(input, increment) {
    ...
    // Set the updated value and calculate the total price
  ...
  document.querySelectorAll('.quantity__minus').forEach(function(btn) {
    btn.addEventListener('click', function(event) {
      ...
      handleQuantityChange(input, false);  // Decrement
    });
  });
  ...
  document.querySelectorAll('.quantity__plus').forEach(function(btn) {
    btn.addEventListener('click', function(event) {
      event.preventDefault();
      const input = this.parentElement.querySelector('.quantity__input');
      handleQuantityChange(input, true);  // Increment
    });
  });

  // Add event listeners for service checkboxes to update the price when selected
  document.querySelectorAll('.services_check').forEach(function(checkbox) {
...
<!-- Include Daterangepicker (add this to your HTML head) -->
